...
static gelu_exact_t input[GELU_LEN];
static gelu_exact_t output[GELU_LEN];
...
static void init_data() {
    for (int i = 0; i < GELU_LEN; ++i) {
        ...
        input[i] = static_cast<gelu_exact_t>(value);
...
int main() {
    init_data();
    exact_gelu_kernel(input, output);
    ...
    std::printf(
        "HLS_METRIC kernel=exact_gelu variant=tanh dtype=float32 shape=len%d mse=%.12e mae=%.12e cosine=%.12f relative_l2=%.12e non_finite=%d\n",
        GELU_LEN,
        mse,
        mae,
        cosine,
