...
static softmax_in_t input[SOFTMAX_ROWS][SOFTMAX_LEN];
static softmax_prob_t output[SOFTMAX_ROWS][SOFTMAX_LEN];
...
static void init_data() {
    for (int r = 0; r < SOFTMAX_ROWS; ++r) {
        for (int c = 0; c < SOFTMAX_LEN; ++c) {
            ...
            input[r][c] = static_cast<softmax_in_t>(value);
        }
    }
}
...
int main() {
    init_data();
    compute_golden();
    lut_softmax_kernel(input, output);
