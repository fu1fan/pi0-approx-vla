...
def main():
    ...
    shapes = [(1, 256, 1024, 1024), (1, 256, 2048, 2048)]
    if args.include_large:
        shapes.append((1, 128, 4096, 4096))
    ...
    for batch, seq, in_dim, out_dim in shapes:
        ...
        for name in ["fp32", "fp16", "int8_fake_quant", "int4_weight_only_fake_quant"]:
            ...
            check_finite(name, y)
            row = {
                "experiment": "linear_quant",
                "shape": f"batch={batch},seq={seq},in={in_dim},out={out_dim}",
                "variant": name,
                "device": device,
    ...
    out_path = Path("results/csv/linear_quant.csv")
